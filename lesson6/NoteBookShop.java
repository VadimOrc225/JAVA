...
public class NoteBookShop {
    ...
    public void addNoteBook(NoteBook notebook) {
        noteBooks.put(noteBooks.size(), notebook); //Добавляем ноутбук в коллекцию
        for (String specKey : notebook.spec.keySet()) { //Цикл добавления характеристик ноутбука в спискок фильтров
            ...
            if (!this.filtersValues.containsKey(specKey)) { //Добавление значения фильтра
                ...
                values.put(1, notebook.spec.get(specKey));
                this.filtersValues.put(specKey, values);
            } else {
                TreeMap<Integer, String> values = this.filtersValues.get(specKey);
                if (!values.containsValue(notebook.spec.get(specKey))) {
                    values.put(values.size() + 1, notebook.spec.get(specKey));
                }
    ...
    public String toString() {
        ...
        for (Integer notebook : noteBooks.keySet()) {
            result.append(this.noteBooks.get(notebook).toString());
        }
    ...
    public void userMenu() {
        System.out.printf("Вас приветствует наш магазин!\n1) Добавить или удалить фильтры\n2) Применить фильтры (выбрано:" +
    ...
    private void applyUserFilters() {
        for (Integer notebookID : this.noteBooks.keySet()) {
            if (this.noteBooks.get(notebookID).filterEquality(this.userFilters)) {
                System.out.print(noteBooks.get(notebookID).toString() + "\n");
            }
        }
    }
