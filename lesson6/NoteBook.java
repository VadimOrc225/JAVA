...
public class NoteBook {
    ...
    public boolean filterEquality(HashMap<String, String> filter) {
        for (String field : filter.keySet()) {
            ...
            if (!filter.get(field).chars().allMatch(Character::isDigit) && Integer.parseInt(this.spec.get(field)) >= Integer.parseInt(filter.get(field)))
                return false;
            else if (!this.spec.get(field).equals(filter.get(field))) return false;
        }
        return true;
    }
